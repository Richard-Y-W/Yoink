...
test('Holo slab builds the Chrome Heart card as sculpted 3D layers', () => {
  ...
  // Front art is grouped; there is no separate back panel or mirrored clone.
  assert.match(slabSource, /cardFront/);
  assert.doesNotMatch(slabSource, /cardFront\.clone\(\)/);
  assert.doesNotMatch(slabSource, /blankBackFace/);
